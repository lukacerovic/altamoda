      ...
      >
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-5">
            ...
            <Link href="/cart" className="relative hover:text-[#edb4bd] transition-colors">
            ...
            <div className="max-w-7xl mx-auto px-4 py-4">
              ...
              {searchResults.length > 0 && (
                <div className="max-w-xl mx-auto mt-3 bg-white rounded-lg border border-[#dddbd9] shadow-xl overflow-hidden">
                  <div className="p-3">
                    <span className="text-[11px] text-[#dddbd9] font-semibold tracking-widest uppercase">Proizvodi</span>
              ...
              {navLinks.map((l) => {
                ...
                return (
                  <div key={l.menuKey || l.name}>
                    <div className="flex items-center justify-between border-b border-[#dddbd9]">
                      <Link
                        href={l.href}
                        onClick={() => setMobileMenu(false)}
                        className="flex-1 py-3 px-2 text-[#1a1c1e] hover:text-[#edb4bd] text-sm font-normal tracking-normal"
                      >
                        {l.name}
                      </Link>
                      {hasMega && (
                        <button
                          ...
                          className="p-2 hover:text-[#edb4bd] transition-colors"
                        >
                          <ChevronDown
                      ...
                      <div className="pl-4 pb-2 animate-slideDown">
                        {brands.map((brand) => (
                          <Link
                            ...
                            href={`/brands/${brand.slug}`}
                            onClick={() => setMobileMenu(false)}
                            className="flex items-center gap-3 py-2 px-2 text-sm text-[#1a1c1e] hover:text-[#edb4bd] transition-colors"
                          >
                            {(() => { const logo = resolveBrandLogo(brand.slug, brand.logoUrl); return logo ? (
                              ...
                              <div className="w-6 h-6 bg-[#FFFFFF] rounded-sm flex items-center justify-center">
                                <span className="text-[10px] font-bold text-[#dddbd9]">{brand.name.charAt(0)}</span>
                              </div>
                            ); })()}
                            {brand.name}
                          </Link>
                      ...
                      <div className="pl-4 pb-2 animate-slideDown">
                        {menuData.columns.map((col) => (
                          <div key={col.title} className="mb-3">
                            <span className="text-xs font-bold uppercase tracking-wider text-[#edb4bd] block py-1 px-2">
                              {col.title}
                            </span>
                            {col.links.map((link) => (
                              <Link
