    ...
    <div className="min-h-screen bg-[#f5f0e8]">
      ...
      <section className="relative h-[300px] md:h-[400px] overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 h-full flex items-center justify-center text-center">
          <div>
            <span className="text-[#8c4a5a] text-xs uppercase tracking-[0.25em] font-medium">Alta Moda Blog</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8">
          <Link href="/" className="hover:text-[#8c4a5a]">Pocetna</Link><ChevronRight className="w-3 h-3" /><span className="text-[#2d2d2d]">Blog</span>
        </nav>

        {/* Category filter pills */}
        <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
          {categories.map((c) => (
            <button key={c} onClick={() => setActiveCategory(c)} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${activeCategory === c ? "bg-[#8c4a5a] text-white" : "bg-white text-gray-600 border border-gray-200 hover:border-[#8c4a5a]"}`}>{c}</button>
          ))}
        </div>
        ...
        <div className="flex gap-8">
              ...
              {filtered.map((post) => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="group bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="p-6">
                    ...
                    <h2 className="text-lg font-bold text-[#2d2d2d] group-hover:text-[#8c4a5a] transition-colors mb-2">{post.title}</h2>
                    <p className="text-sm text-gray-500 line-clamp-2 mb-4">{post.excerpt}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-400">{post.date}</span>
                      <span className="text-sm text-[#8c4a5a] font-medium flex items-center gap-1 group-hover:gap-2 transition-all">Procitaj vise <ArrowRight className="w-3 h-3" /></span>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          <aside className="hidden lg:block w-72 flex-shrink-0 space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="font-semibold text-[#2d2d2d] mb-4">Popularni Clanci</h3>
              <div className="space-y-4">
                {popularPosts.map((p, i) => (
                  <Link key={p.id} href={`/blog/${p.slug}`} className="flex gap-3 group">
                    <span className="text-2xl font-bold text-[#8c4a5a]/30">{String(i + 1).padStart(2, "0")}</span>
                    <div>
                      <h4 className="text-sm font-medium text-[#2d2d2d] group-hover:text-[#8c4a5a] transition-colors line-clamp-2">{p.title}</h4>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="font-semibold text-[#2d2d2d] mb-4">Tagovi</h3>
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 bg-[#faf7f2] text-[#8c4a5a] rounded-full text-xs font-medium hover:bg-[#8c4a5a] hover:text-white transition-colors cursor-pointer">{tag}</span>
                ))}
              </div>
            </div>
          </aside>
