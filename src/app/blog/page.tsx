    ...
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <section className="relative h-[300px] md:h-[400px] overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 h-full flex items-center justify-center text-center">
          <div>
            <span className="text-[#c8a96e] text-xs uppercase tracking-[0.25em] font-medium">Alta Moda Blog</span>
            <h1 className="text-4xl md:text-5xl font-bold mt-4 mb-4 text-white" style={{ fontFamily: "'Playfair Display', serif" }}>Saveti, Trendovi & Inspiracija</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8">
          <Link href="/" className="hover:text-[#c8a96e]">Pocetna</Link><ChevronRight className="w-3 h-3" /><span className="text-[#1a1a1a]">Blog</span>
        </nav>

        {/* Category filter pills */}
        <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
          {categories.map((c) => (
            <button key={c} onClick={() => setActiveCategory(c)} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${activeCategory === c ? "bg-[#c8a96e] text-white" : "bg-white text-gray-600 border border-gray-200 hover:border-[#c8a96e]"}`}>{c}</button>
          ))}
        </div>
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            <div className="grid md:grid-cols-2 gap-6">
              {filtered.map((post) => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="group bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="p-6">
                    ...
                    <h2 className="text-lg font-bold text-[#1a1a1a] group-hover:text-[#c8a96e] transition-colors mb-2">{post.title}</h2>
                    <p className="text-sm text-gray-500 line-clamp-2 mb-4">{post.excerpt}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-400">{post.date}</span>
                      <span className="text-sm text-[#c8a96e] font-medium flex items-center gap-1 group-hover:gap-2 transition-all">Procitaj vise <ArrowRight className="w-3 h-3" /></span>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          <aside className="hidden lg:block w-72 flex-shrink-0 space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="font-semibold text-[#1a1a1a] mb-4">Popularni Clanci</h3>
              <div className="space-y-4">
                {popularPosts.map((p, i) => (
                  <Link key={p.id} href={`/blog/${p.slug}`} className="flex gap-3 group">
                    <span className="text-2xl font-bold text-[#c8a96e]/30">{String(i + 1).padStart(2, "0")}</span>
                    <div>
                      <h4 className="text-sm font-medium text-[#1a1a1a] group-hover:text-[#c8a96e] transition-colors line-clamp-2">{p.title}</h4>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="font-semibold text-[#1a1a1a] mb-4">Tagovi</h3>
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 bg-[#faf7f2] text-[#c8a96e] rounded-full text-xs font-medium hover:bg-[#c8a96e] hover:text-white transition-colors cursor-pointer">{tag}</span>
                ))}
              </div>
            </div>
          </aside>
