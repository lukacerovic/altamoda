...
export default function LoginPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (registerType === "b2b") {
        setSuccess("Vaš B2B zahtev je primljen. Bićete obavešteni kada bude odobren.");
        setLoading(false);
      } else {
        // Auto-login for B2C
        const result = await signIn("credentials", {
      ...
      <div className="max-w-lg mx-auto px-4 py-12 md:py-20">
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="flex border-b border-gray-100">
            <button onClick={() => { setActiveTab("login"); setError(""); setSuccess(""); }} className={`flex-1 py-4 text-sm font-semibold text-center transition-colors ${activeTab === "login" ? "text-[#8c4a5a] border-b-2 border-[#8c4a5a]" : "text-gray-400 hover:text-gray-600"}`}>Prijava</button>
            <button onClick={() => { setActiveTab("register"); setError(""); setSuccess(""); }} className={`flex-1 py-4 text-sm font-semibold text-center transition-colors ${activeTab === "register" ? "text-[#8c4a5a] border-b-2 border-[#8c4a5a]" : "text-gray-400 hover:text-gray-600"}`}>Registracija</button>
          </div>
            ...
            {activeTab === "login" ? (
              ...
              <form onSubmit={handleLogin}>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type={showPassword ? "text" : "password"} value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="Unesite lozinku" className="w-full border border-gray-200 rounded pl-10 pr-10 py-3 text-sm" required />
                ...
                <div className="flex gap-3">
                  <button type="button" className="flex-1 flex items-center justify-center gap-2 border border-gray-200 rounded py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors" onClick={() => alert("Uskoro dostupno")}>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="text" value={regName} onChange={(e) => setRegName(e.target.value)} placeholder="Vase ime" className="w-full border border-gray-200 rounded px-4 py-3 text-sm" required />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">Prezime</label>
                      <input type="text" value={regLastName} onChange={(e) => setRegLastName(e.target.value)} placeholder="Vase prezime" className="w-full border border-gray-200 rounded px-4 py-3 text-sm" required />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Email adresa</label>
                  ...
                  {registerType === "b2b" && (
                    <>
                      ...
                      <div>
                        ...
                        <input type="text" value={regSalonName} onChange={(e) => setRegSalonName(e.target.value)} placeholder="Naziv vaseg salona" className="w-full border border-gray-200 rounded px-4 py-3 text-sm" required />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">PIB</label>
                        <input type="text" value={regPib} onChange={(e) => setRegPib(e.target.value)} placeholder="Poreski identifikacioni broj" className="w-full border border-gray-200 rounded px-4 py-3 text-sm" required />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Maticni broj</label>
                        <input type="text" value={regMaticni} onChange={(e) => setRegMaticni(e.target.value)} placeholder="Maticni broj firme" className="w-full border border-gray-200 rounded px-4 py-3 text-sm" required />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Adresa salona</label>
                        <input type="text" value={regAddress} onChange={(e) => setRegAddress(e.target.value)} placeholder="Ulica i broj, grad" className="w-full border border-gray-200 rounded px-4 py-3 text-sm" />
                      </div>
                    </>
                  )}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Lozinka</label>
                    <input type="password" value={regPassword} onChange={(e) => setRegPassword(e.target.value)} placeholder="Minimum 6 karaktera" className="w-full border border-gray-200 rounded px-4 py-3 text-sm" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Potvrdite lozinku</label>
                    <input type="password" value={regPasswordConfirm} onChange={(e) => setRegPasswordConfirm(e.target.value)} placeholder="Ponovite lozinku" className="w-full border border-gray-200 rounded px-4 py-3 text-sm" required />
                  </div>
                  ...
                  <button type="submit" disabled={loading} className="w-full bg-[#8c4a5a] hover:bg-[#6e3848] text-white py-3 rounded font-medium transition-colors disabled:opacity-50">
                    {loading ? "Obrada..." : registerType === "b2b" ? "Posaljite zahtev za B2B nalog" : "Registrujte se"}
                  </button>
