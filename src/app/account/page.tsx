    ...
    <>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-sm shadow-sm p-12 text-center">
          ...
          <Link href="/products" className="inline-block px-6 py-2.5 bg-[#edb4bd] hover:bg-[#413d3a] text-white rounded text-sm font-medium transition-colors">{t("accountPage.browseProducts")}</Link>
        </div>
      ) : (
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/wishlist" className="text-sm text-[#edb4bd] hover:text-[#1a1c1e] font-medium flex items-center gap-1">
            ...
            <Link key={item.productId} href={`/products/${item.slug}`} className="bg-white rounded-[4px] shadow-sm overflow-hidden group">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <span className="text-sm font-bold text-[#edb4bd]">{item.price.toLocaleString("sr-RS")} RSD</span>
    ...
    <div className="min-h-screen bg-[#FFFFFF] overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-4 py-6">
        <nav className="flex items-center gap-2 text-sm text-[#1a1c1e] mb-6">
          <Link href="/" className="hover:text-[#edb4bd]">{t("accountPage.breadcrumbHome")}</Link><ChevronRight className="w-3 h-3" /><span className="text-[#1a1c1e]">{t("accountPage.breadcrumbAccount")}</span>
        </nav>
        ...
        <div className="flex gap-8 min-w-0">
          ...
          <aside className="hidden lg:block w-64 flex-shrink-0">
            <div className="bg-white rounded-sm shadow-sm p-4 mb-4">
              <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[#dddbd9]">
                ...
                <div>
                  ...
                  <span className="text-xs text-[#edb4bd]">{userLabel}</span>
                </div>
              </div>
              <nav className="space-y-1">
                {sidebarNavKeys.map((item) => {
                  ...
                  return (
                    <button key={item.key} onClick={() => setActiveSection(item.key)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded text-sm font-medium transition-colors ${activeSection === item.key ? "bg-[#FFFFFF] text-[#edb4bd]" : "text-[#1a1c1e] hover:bg-[#FFFFFF] hover:text-[#1a1c1e]"}`}>
                ...
                {(["sr", "en", "ru"] as Language[]).map((lang) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2 rounded text-sm transition-colors ${
                      language === lang
                        ? "bg-[#FFFFFF] text-[#edb4bd] font-medium"
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 text-sm text-[#1a1c1e] hover:text-[#edb4bd] transition-colors py-2"
            >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="lg:hidden flex gap-2 overflow-x-auto pb-4 mb-6">
              {sidebarNavKeys.map((item) => {
                const Icon = item.icon;
                return (
                  <button key={item.key} onClick={() => setActiveSection(item.key)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${activeSection === item.key ? "bg-[#edb4bd] text-white" : "bg-white text-[#1a1c1e] border border-[#dddbd9]"}`}>
