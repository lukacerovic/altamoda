    ...
    <div className="min-h-screen bg-[#FFFBF4]">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-[#11120D] mb-3" style={{ fontFamily: "'Noto Serif', serif" }}>
            Pronađite Salon
          </h1>
          <p className="text-[#7A7F6A]">Otkrijte Alta Moda partnerske salone u vašoj blizini</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-8 max-w-2xl mx-auto">
          <div className="relative flex-1">
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-[#D8CFBC] rounded-sm text-sm focus:border-black transition-all"
            />
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#a5a995]" />
          </div>
          <div className="relative">
            <select
              value={selectedCity}
              onChange={(e) => setSelectedCity(e.target.value)}
              className="appearance-none w-full sm:w-48 px-4 py-3 bg-white border border-[#D8CFBC] rounded-sm text-sm focus:border-black pr-10 cursor-pointer"
            >
              {cities.map((city) => (
                <option key={city} value={city}>{city}</option>
              ))}
            </select>
            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#a5a995] pointer-events-none" />
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            <div className="bg-white rounded-sm border border-[#D8CFBC] overflow-hidden shadow-sm relative" style={{ height: "500px" }}>
              {/* Mock map background */}
              <div className="absolute inset-0 bg-gradient-to-br from-[#e8f4e8] to-[#d4e8d4]">
                ...
                {/* Map label */}
                <div className="absolute top-4 left-4 bg-white/90 px-3 py-1.5 rounded-sm shadow-sm text-xs font-medium text-[#7A7F6A]">
                ...
                {filtered.map((salon) => (
                  ...
                  >
                    <div className={`flex flex-col items-center`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg ${selectedSalon === salon.id ? "bg-black" : salon.partner ? "bg-[#11120D]" : "bg-[#7A7F6A]"}`}>
                        <MapPin className="w-4 h-4 text-white" />
                      </div>
                      <div className={`mt-1 px-2 py-0.5 rounded text-[10px] font-medium whitespace-nowrap ${selectedSalon === salon.id ? "bg-black text-white" : "bg-white text-[#11120D] shadow-sm"}`}>
                        {salon.name}
                      </div>
                    </div>
                  </button>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-lg font-semibold text-[#11120D]">{selected.name}</h3>
                      {selected.partner && (
                    ...
                    <p className="text-sm text-[#7A7F6A] flex items-center gap-1"><MapPin className="w-3 h-3" /> {selected.address}</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <Star className="w-4 h-4 fill-[#7A7F6A] text-secondary" />
                    <span className="font-semibold text-sm">{selected.rating}</span>
                    <span className="text-xs text-[#a5a995]">({selected.reviews})</span>
                  </div>
                </div>
                <div className="flex flex-wrap gap-4 mt-4 text-sm text-[#7A7F6A]">
          ...
          <div className="w-full lg:w-96 space-y-3">
            <p className="text-sm text-[#7A7F6A] mb-2">{filtered.length} salona pronađeno</p>
            {filtered.map((salon) => (
              <button
                key={salon.id}
                onClick={() => setSelectedSalon(salon.id)}
                className={`w-full text-left bg-white rounded-sm border p-4 hover:shadow-sm transition-all ${
                  selectedSalon === salon.id ? "border-black shadow-sm" : "border-[#D8CFBC]"
                }`}
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-sm text-[#11120D]">{salon.name}</h3>
                      {salon.partner && (
                        <span className="px-1.5 py-0.5 bg-black/10 text-secondary text-[9px] font-bold rounded">PARTNER</span>
                      )}
                    </div>
                    <p className="text-xs text-[#a5a995] mt-1 flex items-center gap-1"><MapPin className="w-3 h-3" /> {salon.address}</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <Star className="w-3 h-3 fill-[#7A7F6A] text-secondary" />
                    <span className="text-xs font-semibold">{salon.rating}</span>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-[11px] text-[#a5a995]">
