    ...
    <div className="min-h-screen bg-[#fafafa]">
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-10">
          <HelpCircle className="w-12 h-12 text-[#c8a96e] mx-auto mb-4" />
        ...
        <div className="relative mb-10">
          <input
            ...
            className="w-full pl-12 pr-4 py-4 bg-white border border-[#e5e5e5] rounded-xl text-sm shadow-sm focus:border-[#c8a96e] focus:shadow-md transition-all"
          />
          ...
          {filteredSections.map((section) => (
            <div key={section.title}>
              <h2 className="text-lg font-semibold text-[#1a1a1a] mb-4 flex items-center gap-2">
                <span className="w-1 h-6 bg-[#c8a96e] rounded-full" />
                {section.title}
              </h2>
              <div className="space-y-2">
                {section.items.map((item, idx) => {
                  ...
                  return (
                    <div key={key} className="bg-white rounded-xl border border-[#e5e5e5]/50 overflow-hidden">
                      <button
                      ...
                      >
                        ...
                        <ChevronDown className={`w-5 h-5 text-[#c8a96e] flex-shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`} />
                      </button>
                      {isOpen && (
        ...
        {/* Contact Section */}
        <div className="mt-16 bg-white rounded-2xl border border-[#e5e5e5] p-8 text-center">
          <h3 className="text-xl font-bold text-[#1a1a1a] mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>Niste pronašli odgovor?</h3>
          <p className="text-[#666] mb-6">Naš tim za podršku je tu da vam pomogne</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="tel:+381111234567" className="flex items-center justify-center gap-2 px-6 py-3 border border-[#e5e5e5] rounded-lg text-sm font-medium text-[#333] hover:border-[#c8a96e] hover:text-[#c8a96e] transition-colors">
              <Phone className="w-4 h-4" /> +381 11 123 4567
            </a>
            <a href="mailto:info@altamoda.rs" className="flex items-center justify-center gap-2 px-6 py-3 border border-[#e5e5e5] rounded-lg text-sm font-medium text-[#333] hover:border-[#c8a96e] hover:text-[#c8a96e] transition-colors">
              <Mail className="w-4 h-4" /> info@altamoda.rs
            </a>
            <button className="flex items-center justify-center gap-2 px-6 py-3 bg-[#c8a96e] hover:bg-[#a8894e] text-white rounded-lg text-sm font-medium transition-colors">
