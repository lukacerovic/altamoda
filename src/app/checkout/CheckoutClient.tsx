    ...
    return (
      <div className="min-h-screen bg-[#FFFFFF] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#2e2e2e] mb-4">{t('checkout.emptyCartDesc')}</p>
          ...
          <div className="lg:col-span-2">
            ...
            {step === 'contact' && isGuest && (
              <div className="bg-white rounded-sm shadow-sm p-6">
                ...
                <p className="text-sm text-[#2e2e2e] mb-4">{t('checkout.enterContactDetails')}</p>
                ...
                <div className="mt-4 p-3 bg-[#FFFFFF] rounded-sm text-sm text-[#2e2e2e]">
            ...
            {step === 'address' && (
              <div className="bg-white rounded-sm shadow-sm p-6">
                    ...
                    {addresses.map((addr) => (
                      <label key={addr.id} className={`flex items-start gap-3 p-4 rounded-sm border-2 cursor-pointer transition-colors ${!useNewAddress && selectedAddressId === addr.id ? 'border-black bg-[#FFFFFF]' : 'border-[#D8CFBC] hover:border-[#D8CFBC]'}`}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-[#2e2e2e]">{addr.street}, {addr.postalCode} {addr.city}, {addr.country}</p>
                        </div>
                      </label>
                    ))}
                ...
                <div className="flex gap-3 mt-6">
                  {isGuest && (
                    <button onClick={goPrev} className="flex-1 border border-[#D8CFBC] py-3 rounded font-medium text-[#2e2e2e] hover:bg-[#FFFFFF] flex items-center justify-center gap-2">
                ...
                <div className="bg-white rounded-sm shadow-sm p-6">
                  ...
                  {isGuest && (
                    <div className="mb-4 p-4 bg-[#FFFFFF] rounded-sm">
                      ...
                      <p className="text-sm text-[#2e2e2e]">{guestInfo.name}</p>
                      <p className="text-sm text-[#2e2e2e]">{guestInfo.email} | {guestInfo.phone}</p>
                    </div>
                  )}
                    ...
                    {items.map((item) => (
                      <div key={item.productId} className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-[#2e2e2e]">x{item.quantity}</p>
                        </div>
            ...
            <div className="bg-white rounded-sm shadow-sm p-6 sticky top-24">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-[#2e2e2e]">{t('checkout.subtotal')} ({items.length} {t('checkout.itemsCount')})</span>
                  <span className="font-medium">{subtotal.toLocaleString('sr-RS')} RSD</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#2e2e2e]">{t('checkout.step3')}</span>
              ...
              <div className="mt-4 pt-4 border-t border-[#D8CFBC] space-y-2 text-xs text-[#2e2e2e]">
