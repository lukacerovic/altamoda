    ...
    return (
      <div className="min-h-screen bg-[#FFFBF4] flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-bold text-[#11120D] mb-2">{t('checkout.emptyCart')}</h2>
          <p className="text-[#a5a995] mb-4">{t('checkout.emptyCartDesc')}</p>
          <Link href="/products" className="px-6 py-3 bg-black text-white rounded font-medium hover:bg-[#11120D]">
    ...
    <div className="min-h-screen bg-[#FFFBF4]">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-[#a5a995] mb-6">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#11120D]">{t('checkout.breadcrumbCheckout')}</span>
        </nav>

        <h1 className="text-3xl font-bold text-[#11120D] mb-8" style={{ fontFamily: "'Noto Serif', serif" }}>{t('checkout.breadcrumbCheckout')}</h1>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {step === 'contact' && isGuest && (
              <div className="bg-white rounded-sm shadow-sm p-6">
                <h2 className="text-lg font-bold text-[#11120D] mb-4">{t('checkout.yourDetails')}</h2>
                <p className="text-sm text-[#a5a995] mb-4">{t('checkout.enterContactDetails')}</p>

                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-[#11120D] mb-1">{t('checkout.fullName')} *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#11120D] mb-1">{t('checkout.emailLabel')} *</label>
                    <input
                      type="email"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#11120D] mb-1">{t('checkout.phoneLabel')} *</label>
                    <input
                      type="tel"
                ...
                <div className="mt-4 p-3 bg-[#FFFBF4] rounded-sm text-sm text-[#a5a995]">
                ...
                <button
                  ...
                  className="mt-6 w-full bg-black hover:bg-[#11120D] text-white py-3 rounded font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
            ...
            {step === 'address' && (
              <div className="bg-white rounded-sm shadow-sm p-6">
                <h2 className="text-lg font-bold text-[#11120D] mb-4">{t('checkout.shippingAddress')}</h2>

                {addresses.length > 0 && (
                  <div className="space-y-3 mb-4">
                    {addresses.map((addr) => (
                      <label key={addr.id} className={`flex items-start gap-3 p-4 rounded-sm border-2 cursor-pointer transition-colors ${!useNewAddress && selectedAddressId === addr.id ? 'border-black bg-[#FFFBF4]' : 'border-[#D8CFBC] hover:border-[#D8CFBC]'}`}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-[#a5a995]">{addr.street}, {addr.postalCode} {addr.city}, {addr.country}</p>
                        </div>
                      </label>
                    ))}
                    <label className={`flex items-center gap-3 p-4 rounded-sm border-2 cursor-pointer transition-colors ${useNewAddress ? 'border-black bg-[#FFFBF4]' : 'border-[#D8CFBC] hover:border-[#D8CFBC]'}`}>
                ...
                <div className="flex gap-3 mt-6">
                  {isGuest && (
                    <button onClick={goPrev} className="flex-1 border border-[#D8CFBC] py-3 rounded font-medium text-[#a5a995] hover:bg-[#FFFBF4] flex items-center justify-center gap-2">
                      <ChevronLeft className="w-4 h-4" /> {t('checkout.back')}
                    </button>
                  )}
                  <button onClick={goNext} disabled={!canProceedAddress} className="flex-1 bg-black hover:bg-[#11120D] text-white py-3 rounded font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                    {t('checkout.continue')} <ChevronRight className="w-4 h-4" />
                  </button>
                </div>
              </div>
            ...
            {step === 'shipping' && (
              <div className="bg-white rounded-sm shadow-sm p-6">
                <h2 className="text-lg font-bold text-[#11120D] mb-4">{t('checkout.shippingMethod')}</h2>
                <div className="space-y-3">
                  {[
                    ...
                    <label key={opt.key} className={`flex items-center justify-between gap-3 p-4 rounded-sm border-2 cursor-pointer transition-colors ${shippingMethod === opt.key ? 'border-black bg-[#FFFBF4]' : 'border-[#D8CFBC] hover:border-[#D8CFBC]'}`}>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button onClick={goNext} className="flex-1 bg-black hover:bg-[#11120D] text-white py-3 rounded font-medium flex items-center justify-center gap-2">
                    {t('checkout.continue')} <ChevronRight className="w-4 h-4" />
                  </button>
                </div>
              </div>
            ...
            {step === 'payment' && (
              <div className="bg-white rounded-sm shadow-sm p-6">
                <h2 className="text-lg font-bold text-[#11120D] mb-4">{t('checkout.paymentMethod')}</h2>
                <div className="space-y-3">
                  {paymentOptions.map((opt) => (
                    <label key={opt.key} className={`flex items-center gap-3 p-4 rounded-sm border-2 cursor-pointer transition-colors ${paymentMethod === opt.key ? 'border-black bg-[#FFFBF4]' : 'border-[#D8CFBC] hover:border-[#D8CFBC]'}`}>
                ...
                <div className="mt-4">
                  <label className="block text-sm font-medium text-[#11120D] mb-2">{t('checkout.noteOptional')}</label>
            ...
            {step === 'review' && (
              <div className="space-y-4">
                <div className="bg-white rounded-sm shadow-sm p-6">
                  <h2 className="text-lg font-bold text-[#11120D] mb-4">{t('checkout.orderReview')}</h2>

                  {/* Guest contact summary */}
                  {isGuest && (
                    <div className="mb-4 p-4 bg-[#FFFBF4] rounded-sm">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-semibold text-[#11120D]">{t('checkout.contactDetails')}</span>
                        <button onClick={() => setStep('contact')} className="text-xs text-secondary hover:underline">{t('checkout.edit')}</button>
                      </div>
                      <p className="text-sm text-[#a5a995]">{guestInfo.name}</p>
                      <p className="text-sm text-[#a5a995]">{guestInfo.email} | {guestInfo.phone}</p>
                    </div>
                  )}
                  ...
                  <div className="mb-4 p-4 bg-[#FFFBF4] rounded-sm">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-semibold text-[#11120D]">{t('checkout.shippingAddress')}</span>
                  ...
                  <div className="mb-4 p-4 bg-[#FFFBF4] rounded-sm">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-semibold text-[#11120D]">{t('checkout.step3')}</span>
                  ...
                  {/* Payment summary */}
                  <div className="p-4 bg-[#FFFBF4] rounded-sm">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-semibold text-[#11120D]">{t('checkout.step4')}</span>
                ...
                {/* Items */}
                <div className="bg-white rounded-sm shadow-sm p-6">
                  <h3 className="text-sm font-semibold text-[#11120D] mb-4">{t('checkout.products')} ({items.length})</h3>
                  <div className="space-y-3">
                    {items.map((item) => (
                      <div key={item.productId} className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded overflow-hidden flex-shrink-0 bg-[#FFFBF4]">
                          {item.image && <Image src={item.image} alt={item.name} width={64} height={64} className="w-full h-full object-cover" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-[#11120D] truncate">{item.name}</p>
                          <p className="text-xs text-[#a5a995]">x{item.quantity}</p>
                        </div>
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-sm p-3 text-sm text-red-700">
                    ...
                    {isGuest && error.includes(t('checkout.mustBeLoggedIn').substring(0, 10)) && (
                      <div className="mt-2 flex gap-2">
                        <Link href="/account/login?callbackUrl=/checkout" className="px-4 py-2 bg-black text-white text-sm rounded font-medium hover:bg-[#11120D]">
                          {t('checkout.loginLink')}
                        </Link>
                        <Link href="/account/register?callbackUrl=/checkout" className="px-4 py-2 border border-black text-secondary text-sm rounded font-medium hover:bg-[#FFFBF4]">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handlePlaceOrder} disabled={isSubmitting || !b2bMinimumMet} className="flex-1 bg-black hover:bg-[#11120D] text-white py-3.5 rounded font-medium disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          <div>
            <div className="bg-white rounded-sm shadow-sm p-6 sticky top-24">
              <h3 className="text-lg font-bold text-[#11120D] mb-4" style={{ fontFamily: "'Noto Serif', serif" }}>{t('checkout.summary')}</h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-[#a5a995]">{t('checkout.subtotal')} ({items.length} {t('checkout.itemsCount')})</span>
                  <span className="font-medium">{subtotal.toLocaleString('sr-RS')} RSD</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#a5a995]">{t('checkout.step3')}</span>
              ...
              <div className="mt-4 pt-4 border-t border-[#D8CFBC] space-y-2 text-xs text-[#a5a995]">
