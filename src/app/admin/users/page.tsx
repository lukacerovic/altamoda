...
export default function UsersPage() {
  ...
  const roleBadge = (role: string) => {
    switch (role) {
      ...
      case "b2c": return <span className="inline-block px-2.5 py-1 rounded-full text-xs font-medium bg-stone-100 text-[#837A64]">B2C</span>;
  ...
  return (
    <div className="space-y-6 max-w-full">
      ...
      <div>
        ...
        <p className="text-sm text-[#837A64] mt-1">{total} {t("admin.registeredUsers")}</p>
      </div>
      ...
      <div className="bg-white rounded-sm border border-stone-200 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button onClick={() => setShowFilters(!showFilters)} className="sm:hidden flex items-center gap-2 px-4 py-2.5 bg-stone-100 rounded-sm text-sm text-[#837A64]">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2.5 rounded-sm text-sm font-medium transition-colors ${
                    roleFilter === opt.value ? "bg-stone-900 text-white" : "bg-stone-100 text-[#837A64] hover:bg-[#D8CFBC]"
                  }`}
                >
                  {opt.label}
                </button>
              ...
              <table className="w-full min-w-[750px]">
                <thead>
                  <tr className="bg-stone-100 border-b border-stone-200">
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#837A64] uppercase tracking-wider">{t("admin.user")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#837A64] uppercase tracking-wider">{t("admin.type")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#837A64] uppercase tracking-wider">{t("admin.registration")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#837A64] uppercase tracking-wider">{t("admin.orders")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#837A64] uppercase tracking-wider">{t("admin.spent")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#837A64] uppercase tracking-wider">{t("admin.status")}</th>
                ...
                <tbody className="divide-y divide-[#FFFFFF]">
                  {users.length === 0 ? (
                    ...
                    <tr key={user.id} className={`hover:bg-stone-100 transition-colors cursor-pointer ${selectedUser?.id === user.id ? "bg-black/5" : ""}`} onClick={() => setSelectedUser(user)}>
                      ...
                      <td className="px-6 py-4 text-sm text-[#837A64]">{formatDate(user.createdAt)}</td>
          ...
          {totalPages > 1 && (
            <div className="px-6 py-4 border-t border-stone-200 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="p-2 rounded-sm text-[#837A64] hover:bg-stone-100 disabled:opacity-30 disabled:cursor-not-allowed">
                  <ChevronLeft size={18} />
                </button>
                {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                  ...
                  return (
                    <button key={page} onClick={() => setCurrentPage(page)} className={`w-9 h-9 rounded-sm text-sm font-medium transition-colors ${page === currentPage ? "bg-black text-white" : "text-[#837A64] hover:bg-stone-100"}`}>
                      {page}
                    </button>
                  );
                })}
                <button onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="p-2 rounded-sm text-[#837A64] hover:bg-stone-100 disabled:opacity-30 disabled:cursor-not-allowed">
