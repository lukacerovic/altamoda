...
export default function UsersPage() {
  ...
  const roleBadge = (role: string) => {
    switch (role) {
      ...
      case "b2c": return <span className="inline-block px-2.5 py-1 rounded-full text-xs font-medium bg-stone-100 text-[#2e2e2e]">B2C</span>;
  ...
  return (
    <div className="space-y-6 max-w-full">
      ...
      <div>
        ...
        <p className="text-sm text-[#2e2e2e] mt-1">{total} {t("admin.registeredUsers")}</p>
      </div>
      ...
      <div className="bg-white rounded-sm border border-stone-200 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button onClick={() => setShowFilters(!showFilters)} className="sm:hidden flex items-center gap-2 px-4 py-2.5 bg-stone-100 rounded-sm text-sm text-[#2e2e2e]">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2.5 rounded-sm text-sm font-medium transition-colors ${
                    roleFilter === opt.value ? "bg-stone-900 text-white" : "bg-stone-100 text-[#2e2e2e] hover:bg-[#D8CFBC]"
                  }`}
                >
                  {opt.label}
                </button>
              ...
              <table className="w-full min-w-[750px]">
                <thead>
                  <tr className="bg-stone-100 border-b border-stone-200">
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">{t("admin.user")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">{t("admin.type")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">{t("admin.registration")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">{t("admin.orders")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">{t("admin.spent")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">{t("admin.status")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#FFFFFF]">
                  {users.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-6 py-12 text-center text-sm text-[#2e2e2e]">
                    ...
                    <tr key={user.id} className={`hover:bg-stone-100 transition-colors cursor-pointer ${selectedUser?.id === user.id ? "bg-black/5" : ""}`} onClick={() => setSelectedUser(user)}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${user.status === "pending" ? "bg-yellow-50 text-yellow-600" : "bg-stone-100 text-[#2e2e2e]"}`}>
                            <User size={16} />
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-black truncate">{user.name}</p>
                            <p className="text-xs text-[#2e2e2e] truncate">{user.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">{roleBadge(user.role)}</td>
                      <td className="px-6 py-4 text-sm text-[#2e2e2e]">{formatDate(user.createdAt)}</td>
                      <td className="px-6 py-4 text-sm text-[#2e2e2e]">{user.ordersCount}</td>
                      ...
                      <td className="px-6 py-4">
                        <button className="p-1.5 text-[#2e2e2e] hover:text-secondary rounded-sm transition-colors">
                          <Eye size={16} />
                        </button>
                      </td>
                    </tr>
          ...
          {totalPages > 1 && (
            <div className="px-6 py-4 border-t border-stone-200 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="p-2 rounded-sm text-[#2e2e2e] hover:bg-stone-100 disabled:opacity-30 disabled:cursor-not-allowed">
                  <ChevronLeft size={18} />
                </button>
                {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                  ...
                  return (
                    <button key={page} onClick={() => setCurrentPage(page)} className={`w-9 h-9 rounded-sm text-sm font-medium transition-colors ${page === currentPage ? "bg-black text-white" : "text-[#2e2e2e] hover:bg-stone-100"}`}>
                      {page}
                    </button>
                  );
                })}
                <button onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="p-2 rounded-sm text-[#2e2e2e] hover:bg-stone-100 disabled:opacity-30 disabled:cursor-not-allowed">
        ...
        {selectedUser && (
          <div className="w-full lg:w-96 bg-white rounded-sm border border-stone-200 overflow-hidden flex-shrink-0">
            <div className="p-6 border-b border-stone-200 flex items-center justify-between">
              ...
              <button onClick={() => setSelectedUser(null)} className="p-1 text-[#2e2e2e] hover:text-black">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-sm">
                  <Mail size={14} className="text-[#2e2e2e]" />
                  <span className="text-[#2e2e2e]">{selectedUser.email}</span>
                </div>
                {selectedUser.phone && (
                  <div className="flex items-center gap-2 text-sm">
                    <Phone size={14} className="text-[#2e2e2e]" />
                    <span className="text-[#2e2e2e]">{selectedUser.phone}</span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-sm">
                  <Calendar size={14} className="text-[#2e2e2e]" />
              ...
              {selectedUser.b2bProfile && (
                <div className="p-4 rounded-sm bg-stone-100 border border-stone-200">
                  <h5 className="text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider mb-3">{t("admin.b2bData")}</h5>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      <Building size={14} className="text-[#2e2e2e]" />
                      <span className="text-[#2e2e2e]">{selectedUser.b2bProfile.salonName}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-xs font-medium text-[#2e2e2e] w-[14px] text-center">PIB</span>
                      <span className="text-[#2e2e2e]">{selectedUser.b2bProfile.pib}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Hash size={14} className="text-[#2e2e2e]" />
                      <span className="text-[#2e2e2e]">{t("admin.maticniBroj")}: {selectedUser.b2bProfile.maticniBroj}</span>
                    </div>
                    {selectedUser.b2bProfile.address && (
                      <div className="flex items-center gap-2 text-sm">
                        <MapPin size={14} className="text-[#2e2e2e]" />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-sm bg-stone-100 text-center">
                  ...
                  <p className="text-xs text-[#2e2e2e]">{t("admin.orders")}</p>
                </div>
                <div className="p-3 rounded-sm bg-stone-100 text-center">
                  ...
                  <p className="text-xs text-[#2e2e2e]">{t("admin.rsdSpent")}</p>
                </div>
              </div>
