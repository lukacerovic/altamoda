  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-serif text-2xl lg:text-3xl font-bold text-[#1a1a1a]">Import / Export</h1>
      ...
      <div className="flex gap-1 bg-[#f5f5f5] rounded-lg p-1 mb-6 w-fit">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-5 py-2 rounded-md text-sm font-medium transition-all ${activeTab === tab.id ? "bg-white text-[#1a1a1a] shadow-sm" : "text-[#666] hover:text-[#333]"}`}
          >
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === "import" && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
            <h3 className="font-semibold text-[#1a1a1a] mb-3">Tip uvoza</h3>
            <select
              value={importType}
          ...
          {/* Template downloads */}
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
            <h3 className="font-semibold text-[#1a1a1a] mb-3">Preuzmite šablon</h3>
            <div className="flex flex-wrap gap-3">
              {["Preuzmite šablon za proizvode", "Preuzmite šablon za cene", "Preuzmite šablon za lager"].map((tmpl) => (
                <button key={tmpl} className="inline-flex items-center gap-2 px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm hover:bg-[#f5f5f5] transition-colors">
                  <Download size={16} className="text-[#c8a96e]" />
                  {tmpl}
                </button>
              ))}
            </div>
          ...
          <div
            ...
            className={`bg-white rounded-xl border-2 border-dashed p-10 text-center cursor-pointer transition-colors ${fileUploaded ? "border-green-300 bg-green-50" : "border-[#e5e5e5] hover:border-[#c8a96e]"}`}
          >
            {fileUploaded ? (
              <div>
              ...
              <div>
                <Upload size={40} className="mx-auto text-[#c8a96e] mb-3" />
            ...
            <>
              <div className="bg-white rounded-xl border border-[#e5e5e5] overflow-hidden">
                <div className="p-4 border-b border-[#e5e5e5]">
                  <h3 className="font-semibold text-[#1a1a1a]">Pregled podataka (prvih 5 redova)</h3>
                </div>
              ...
              {/* Field Mapping */}
              <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
                <h3 className="font-semibold text-[#1a1a1a] mb-4">Mapiranje polja</h3>
                <div className="space-y-3">
                  {mappings.map((mapping, index) => (
                    <div key={index} className="flex items-center gap-3">
                      <div className="flex-1 px-4 py-2 bg-[#fafafa] border border-[#e5e5e5] rounded-lg text-sm text-[#333]">
                        {mapping.csv}
                      </div>
                      <ArrowRight size={16} className="text-[#c8a96e] flex-shrink-0" />
                      <select
                        value={mapping.system}
                        onChange={(e) => updateMapping(index, e.target.value)}
                        className="flex-1 px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none"
                      >
                        {systemFields.map((f) => (
              ...
              <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
                <div className="flex items-center justify-between mb-4">
                  <button
                    ...
                    className="inline-flex items-center gap-2 bg-[#c8a96e] text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-[#b8994e] transition-colors"
                  >
                    <Upload size={16} />
                    Pokreni import
                  </button>
                ...
                <div className="w-full bg-[#f5f5f5] rounded-full h-3">
                  <div
                    className="bg-[#c8a96e] h-3 rounded-full transition-all duration-500"
          ...
          <div className="bg-white rounded-xl border border-[#e5e5e5] overflow-hidden">
            <div className="p-4 border-b border-[#e5e5e5]">
              <h3 className="font-semibold text-[#1a1a1a]">Istorija uvoza</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-[#fafafa] border-b border-[#e5e5e5]">
      ...
      {activeTab === "export" && (
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
            <h3 className="font-semibold text-[#1a1a1a] mb-4">Podešavanja izvoza</h3>
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">Tip podataka</label>
                  <select value={exportType} onChange={(e) => setExportType(e.target.value)} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none">
                    <option value="Proizvodi">Proizvodi</option>
                ...
                <div>
                  ...
                  <select value={exportFormat} onChange={(e) => setExportFormat(e.target.value)} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none">
              ...
              {exportType === "Porudžbine" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="date" value={exportDateFrom} onChange={(e) => setExportDateFrom(e.target.value)} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#333] mb-1">Datum do</label>
                    <input type="date" value={exportDateTo} onChange={(e) => setExportDateTo(e.target.value)} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" />
                  </div>
                </div>
              )}
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  <select className="px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none">
          ...
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-[#1a1a1a]">Spreman za izvoz</h3>
