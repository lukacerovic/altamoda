      ...
      <div className="border-b border-stone-200 bg-white">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center gap-4">
          <Link href="/admin/products" className="p-2 hover:bg-stone-100 rounded-sm transition-colors">
            <ArrowLeft size={18} className="text-[#293133]/65" />
          </Link>
          <div>
            <h1 className="text-lg font-semibold text-black">Uvoz proizvoda</h1>
            <p className="text-sm text-[#293133]/65">Uvezite proizvode, kategorije i barkodove iz jednog ili više fajlova</p>
          </div>
        </div>
      </div>
        ...
        {!result && (
          <>
            ...
            >
              ...
              <p className="text-sm text-[#293133]/65">Prevucite fajlove ovde ili <span className="text-black font-medium underline">izaberite fajlove</span></p>
              <p className="text-xs text-[#293133]/65 mt-1">CSV, XLSX ili XLS • Više fajlova odjednom • Maksimalno 10MB po fajlu</p>
            </div>

            {/* Selected files list */}
            {files.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs text-[#293133]/65 font-medium uppercase tracking-wider">{files.length} {files.length === 1 ? 'fajl izabran' : 'fajlova izabrano'}</p>
                {files.map((f, i) => (
                  <div key={f.name} className="flex items-center gap-3 bg-white border border-stone-200 rounded-sm px-4 py-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[#293133]/65">{formatSize(f.size)}</p>
                    </div>
                    <button onClick={() => removeFile(i)} className="p-1 hover:bg-stone-100 rounded-sm">
                      <X size={16} className="text-[#293133]/65" />
                    </button>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-white border border-stone-200 rounded-sm p-4 text-center">
                ...
                <p className="text-xs text-[#293133]/65 mt-1">Kreirano</p>
              </div>
              <div className="bg-white border border-stone-200 rounded-sm p-4 text-center">
                <p className="text-2xl font-bold text-blue-600">{result.totals.updated}</p>
                <p className="text-xs text-[#293133]/65 mt-1">Ažurirano</p>
              </div>
              <div className="bg-white border border-stone-200 rounded-sm p-4 text-center">
                <p className="text-2xl font-bold text-[#293133]/65">{result.totals.skipped}</p>
                <p className="text-xs text-[#293133]/65 mt-1">Preskočeno</p>
              </div>
              <div className="bg-white border border-stone-200 rounded-sm p-4 text-center">
                <p className={`text-2xl font-bold ${result.totals.errors > 0 ? "text-red-500" : "text-[#293133]/65"}`}>{result.totals.errors}</p>
                <p className="text-xs text-[#293133]/65 mt-1">Grešaka</p>
              </div>
            </div>
                ...
                <div key={idx} className="bg-white border border-stone-200 rounded-sm overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[#293133]/65">{fr.fileLabel} • {fr.rows} redova</p>
                    </div>
                    <div className="flex items-center gap-3 text-xs">
                      ...
                      {fr.skipped > 0 && <span className="text-[#293133]/65">-{fr.skipped}</span>}
                      {fr.errors.length > 0 && <span className="text-red-500 font-medium">{fr.errors.length} gr.</span>}
                    </div>
                    {isExpanded ? <ChevronUp size={16} className="text-[#293133]/65" /> : <ChevronDown size={16} className="text-[#293133]/65" />}
                  </button>
                        ...
                        <div className="max-h-48 overflow-y-auto">
                          {fr.errors.map((err, i) => (
                            <div key={i} className="py-1.5 border-b border-stone-100 last:border-0 text-xs">
                              <span className="text-[#293133]/65 mr-1">Red {err.row}</span>
