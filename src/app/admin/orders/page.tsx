...
export default function OrdersPage() {
  ...
  const getTimelineIcon = (status: string) => {
    switch (status) {
      ...
      default:
        return <Clock size={16} className="text-[#1a1c1e]" />;
    }
  };
    ...
    <div className="space-y-6 max-w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#1a1c1e] mt-1">
      ...
      <div className="bg-white rounded-sm border border-stone-200 p-4">
        ...
        {/* Status tabs */}
        <div className={`mt-3 pt-3 border-t border-[#FFFFFF] ${showFilters ? "block" : "hidden"} sm:block`}>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-sm text-sm font-medium transition-colors ${
                  statusFilter === s.key
                    ...
                    : "bg-stone-100 text-[#1a1c1e] hover:bg-[#dddbd9]"
                }`}
              >
                {s.label}
              </button>
      ...
      <div className="bg-white rounded-sm border border-stone-200 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            <Package size={40} className="mx-auto text-[#dddbd9] mb-3" />
            <p className="text-sm text-[#1a1c1e]">{t("admin.noOrdersMatch")}</p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[800px]">
              <thead>
                <tr className="bg-stone-100 border-b border-stone-200">
                  <th className="px-6 py-3 text-left text-xs font-semibold text-[#1a1c1e] uppercase tracking-wider">
                    {t("admin.orderNo")}
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-[#1a1c1e] uppercase tracking-wider">
                    {t("admin.customer")}
                  ...
                  <Fragment key={order.id}>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        <p className="text-xs text-[#1a1c1e] md:hidden">{formatDate(order.createdAt)}</p>
                      </td>
                              ...
                              {VALID_TRANSITIONS[order.status].map((s) => (
                                <button
                                  ...
                                  className={`w-full text-left px-3 py-2 text-sm hover:bg-stone-50 flex items-center gap-2 transition-colors ${
                                    s === "otkazano" ? "text-red-600" : s === "isporuceno" ? "text-emerald-600" : "text-[#1a1c1e]"
                                  }`}
                                >
                      ...
                      <td className="px-6 py-4">
                        <button className="text-[#1a1c1e] hover:text-black transition-colors">
                              ...
                              <div className="lg:col-span-2">
                                  ...
                                  <table className="w-full">
                                    <thead>
                                      <tr className="bg-stone-100">
                                        <th className="px-4 py-2 text-left text-xs font-medium text-[#1a1c1e]">
                                          {t("admin.product")}
                                        </th>
                                        <th className="px-4 py-2 text-left text-xs font-medium text-[#1a1c1e]">
                                          {t("admin.qty")}
                                        ...
                                        <tr key={item.id}>
                                          <td className="px-4 py-2.5">
                                            <div className="flex items-center gap-3">
                                              ...
                                              <div>
                                                <p className="text-sm text-[#1a1c1e]">{item.productName}</p>
                                                <p className="text-[10px] text-[#1a1c1e]">{item.productSku}</p>
                                              </div>
                                            </div>
                                          </td>
                                          <td className="px-4 py-2.5 text-sm text-[#1a1c1e]">
                                            {item.quantity}
                                          </td>
                                          <td className="px-4 py-2.5 text-sm text-[#1a1c1e]">
                                    ...
                                    <tfoot>
                                      {orderDetail.discountAmount > 0 && (
                                        <tr className="border-t border-stone-200">
                                          <td colSpan={3} className="px-4 py-2 text-sm text-[#1a1c1e] text-right">
                                            Popust:
                                          </td>
                                      ...
                                      {orderDetail.shippingCost > 0 && (
                                        <tr>
                                          ...
                                          <td className="px-4 py-2 text-sm text-[#1a1c1e]">
                                ...
                                <div className="bg-white rounded-sm border border-stone-200 p-4 space-y-2">
                                  ...
                                  {orderDetail.shippingMethod && (
                                    <div className="flex items-center gap-2 text-sm text-[#1a1c1e]">
                                      <Truck size={14} className="text-[#1a1c1e]" />{" "}
                                  ...
                                  {orderDetail.notes && (
                                    <div className="flex items-start gap-2 text-sm text-[#1a1c1e] mt-2 pt-2 border-t border-stone-100">
                              ...
                              <div>
                                ...
                                <div className="bg-white rounded-sm border border-stone-200 p-4">
                                  {orderDetail.statusHistory.length === 0 ? (
                                    <p className="text-sm text-[#1a1c1e] text-center py-4">
                                        ...
                                        <div key={event.id} className="flex gap-3">
                                          <div className="flex flex-col items-center">
                                            ...
                                            {idx < orderDetail.statusHistory.length - 1 && (
                                              <div className="w-px h-full bg-[#dddbd9] mt-1" />
                                            )}
                                          </div>
                                          <div className="pb-4">
                                            <p className="text-sm text-[#1a1c1e] font-medium">
                                              {statusLabel(event.status)}
                                            </p>
                                            {event.note && (
                                              <p className="text-xs text-[#1a1c1e] mt-0.5">{event.note}</p>
                                            )}
                                            <p className="text-xs text-[#1a1c1e] mt-0.5">
        ...
        {totalPages > 1 && (
          <div className="px-4 sm:px-6 py-4 border-t border-stone-200 flex items-center justify-between gap-2">
            <span className="text-xs sm:text-sm text-[#1a1c1e] whitespace-nowrap">
