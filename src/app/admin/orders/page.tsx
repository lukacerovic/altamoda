...
export default function OrdersPage() {
  ...
  const getTimelineIcon = (status: string) => {
    switch (status) {
      ...
      default:
        return <Clock size={16} className="text-[#2e2e2e]" />;
    }
  };
              ...
              <button
                ...
                className={`px-4 py-2 rounded-sm text-sm font-medium transition-colors ${
                  statusFilter === s.key
                    ...
                    : "bg-stone-100 text-[#2e2e2e] hover:bg-[#D8CFBC]"
                }`}
              >
                {s.label}
              </button>
      ...
      <div className="bg-white rounded-sm border border-stone-200 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-[#2e2e2e]">{t("admin.noOrdersMatch")}</p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[800px]">
              <thead>
                <tr className="bg-stone-100 border-b border-stone-200">
                  <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">
                    {t("admin.orderNo")}
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">
                    {t("admin.customer")}
                  ...
                  <Fragment key={order.id}>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        {order.user?.role === "b2b" && (
                          <span className="text-[10px] font-semibold bg-stone-200 text-[#2e2e2e] px-1.5 py-0.5 rounded">
                            B2B
                          </span>
                        )}
                        <p className="text-xs text-[#2e2e2e] md:hidden">{formatDate(order.createdAt)}</p>
                      </td>
                              ...
                              {VALID_TRANSITIONS[order.status].map((s) => (
                                <button
                                  ...
                                  className={`w-full text-left px-3 py-2 text-sm hover:bg-stone-50 flex items-center gap-2 transition-colors ${
                                    s === "otkazano" ? "text-red-600" : s === "isporuceno" ? "text-emerald-600" : "text-[#2e2e2e]"
                                  }`}
                                >
                      ...
                      <td className="px-6 py-4">
                        <button className="text-[#2e2e2e] hover:text-black transition-colors">
                              ...
                              <div className="lg:col-span-2">
                                  ...
                                  <table className="w-full">
                                    <thead>
                                      <tr className="bg-stone-100">
                                        <th className="px-4 py-2 text-left text-xs font-medium text-[#2e2e2e]">
                                          {t("admin.product")}
                                        </th>
                                        <th className="px-4 py-2 text-left text-xs font-medium text-[#2e2e2e]">
                                          {t("admin.qty")}
                                          ...
                                          <td className="px-4 py-2.5">
                                            <div className="flex items-center gap-3">
                                              ...
                                              <div>
                                                ...
                                                <p className="text-[10px] text-[#2e2e2e]">{item.productSku}</p>
                                              </div>
                                            </div>
                                          </td>
                                          <td className="px-4 py-2.5 text-sm text-[#2e2e2e]">
                                            {item.quantity}
                                          </td>
                                          <td className="px-4 py-2.5 text-sm text-[#2e2e2e]">
                                    ...
                                    <tfoot>
                                      {orderDetail.discountAmount > 0 && (
                                        <tr className="border-t border-stone-200">
                                          <td colSpan={3} className="px-4 py-2 text-sm text-[#2e2e2e] text-right">
                                            Popust:
                                          </td>
                                      ...
                                      {orderDetail.shippingCost > 0 && (
                                        <tr>
                                          ...
                                          <td className="px-4 py-2 text-sm text-[#2e2e2e]">
                                ...
                                <div className="bg-white rounded-sm border border-stone-200 p-4 space-y-2">
                                      ...
                                      {orderDetail.user.name && (
                                        <div className="flex items-center gap-2 text-sm text-[#2e2e2e]">
                                          ...
                                          {orderDetail.user.role === "b2b" && (
                                            <span className="text-[10px] font-semibold bg-stone-200 text-[#2e2e2e] px-1.5 py-0.5 rounded ml-1">
                                              B2B
                                            </span>
                                          )}
                                        </div>
                                  ...
                                  {orderDetail.shippingMethod && (
                                    <div className="flex items-center gap-2 text-sm text-[#2e2e2e]">
                                      <Truck size={14} className="text-[#2e2e2e]" />{" "}
                                  ...
                                  {orderDetail.notes && (
                                    <div className="flex items-start gap-2 text-sm text-[#2e2e2e] mt-2 pt-2 border-t border-stone-100">
                                      ...
                                      {orderDetail.statusHistory.map((event, idx) => (
                                        <div key={event.id} className="flex gap-3">
                                          ...
                                          <div className="pb-4">
                                            ...
                                            {event.note && (
                                              <p className="text-xs text-[#2e2e2e] mt-0.5">{event.note}</p>
                                            )}
        ...
        {totalPages > 1 && (
          <div className="px-4 sm:px-6 py-4 border-t border-stone-200 flex items-center justify-between gap-2">
            <span className="text-xs sm:text-sm text-[#2e2e2e] whitespace-nowrap">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  ...
                  className={`w-8 h-8 sm:w-9 sm:h-9 rounded-sm text-sm font-medium transition-colors ${
                    page === currentPage
                      ? "bg-black text-white"
                      : "text-[#2e2e2e] hover:bg-stone-100"
                  }`}
                >
                  {page}
                </button>
