...
import {
  ...
  Users, ShoppingCart, ListOrdered, X,
} from "lucide-react";
...
const FILE_TYPE_ICONS: Record<string, typeof Users> = {
  pantheon_customers: Users,
  pantheon_orders: ShoppingCart,
  pantheon_order_items: ListOrdered,
};

const FILE_TYPE_COLORS: Record<string, string> = {
  pantheon_customers: "text-purple-600 bg-purple-50",
  pantheon_orders: "text-green-600 bg-green-50",
  pantheon_order_items: "text-blue-600 bg-blue-50",
};
    ...
    <div className="min-h-screen bg-[#faf8f4]">
      ...
      <div className="border-b border-stone-200 bg-white">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center gap-4">
          <Link href="/admin/orders" className="p-2 hover:bg-stone-100 rounded-sm transition-colors">
            <ArrowLeft size={18} className="text-[#666]" />
          </Link>
          <div>
            <h1 className="text-lg font-semibold text-black">Uvoz porudžbina</h1>
            <p className="text-sm text-[#999]">Uvezite kupce, porudžbine i stavke iz Pantheon ERP fajlova</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-sm p-4 flex gap-3">
          ...
          <div className="text-sm text-blue-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 text-blue-700">
              <div className="flex items-center gap-2"><Users size={14} /> <span><strong>the_setSubj</strong> — Kupci</span></div>
              <div className="flex items-center gap-2"><ShoppingCart size={14} /> <span><strong>tHE_Order</strong> — Porudžbine</span></div>
              <div className="flex items-center gap-2"><ListOrdered size={14} /> <span><strong>tHE_Orderitem</strong> — Stavke</span></div>
            </div>
            <p className="mt-2 text-blue-600">Izaberite jedan ili više fajlova — sistem automatski prepoznaje tip i obrađuje ih pravilnim redosledom (kupci → porudžbine → stavke).</p>
            <p className="mt-1 text-blue-600"><strong>Važno:</strong> Proizvodi moraju biti uvezeni pre stavki porudžbina.</p>
          </div>
        </div>
