  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-serif text-2xl lg:text-3xl font-bold text-[#1a1a1a]">Baneri</h1>
          <p className="text-[#666] mt-1">Upravljajte banerima, pop-up-ovima i hero sekcijama</p>
        </div>
        <button onClick={openCreate} className="inline-flex items-center gap-2 bg-[#c8a96e] text-white px-5 py-2.5 rounded-lg hover:bg-[#b8994e] transition-colors font-medium text-sm">
          <Plus size={18} />
          Novi baner
        </button>
      </div>
      ...
      <div className="space-y-4">
        {sorted.map((banner) => (
          <div key={banner.id} className="bg-white rounded-xl border border-[#e5e5e5] overflow-hidden hover:shadow-md transition-shadow">
            <div className="flex flex-col md:flex-row">
              {/* Preview */}
              <div className="w-full md:w-56 h-32 md:h-auto bg-gradient-to-br from-[#c8a96e]/20 to-[#c8a96e]/5 flex items-center justify-center flex-shrink-0 relative">
                <Image size={32} className="text-[#c8a96e]/40" />
              ...
              <div className="flex-1 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-semibold text-[#1a1a1a] mb-2">{banner.title}</h3>
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      {typeBadge(banner.type)}
                      <span className="text-xs bg-[#f5f5f5] text-[#666] px-2 py-0.5 rounded">{banner.position}</span>
                      <span className="text-xs bg-[#f5f5f5] text-[#666] px-2 py-0.5 rounded">{banner.target}</span>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <div className="flex flex-col">
                      <button onClick={() => moveOrder(banner.id, "up")} className="p-1 hover:bg-[#f5f5f5] rounded transition-colors">
                        <ChevronUp size={14} className="text-[#666]" />
                      </button>
                      <button onClick={() => moveOrder(banner.id, "down")} className="p-1 hover:bg-[#f5f5f5] rounded transition-colors">
                        <ChevronDown size={14} className="text-[#666]" />
                      </button>
                    </div>
                    <button onClick={() => toggleStatus(banner.id)} className="p-1.5 hover:bg-[#f5f5f5] rounded-lg transition-colors">
                      {banner.status === "active" ? <ToggleRight size={22} className="text-green-600" /> : <ToggleLeft size={22} className="text-[#999]" />}
                    </button>
                    <button onClick={() => openEdit(banner)} className="p-1.5 hover:bg-[#f5f5f5] rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-[#e5e5e5]">
              <h2 className="font-serif text-xl font-bold text-[#1a1a1a]">{editingBanner ? "Izmeni baner" : "Novi baner"}</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">Tip</label>
                  <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as Banner["type"] })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none">
                ...
                <div>
                  ...
                  <select value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value as Banner["position"] })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.desktopImage} onChange={(e) => setForm({ ...form, desktopImage: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" placeholder="/banners/desktop.jpg" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">Mobile slika URL</label>
                  <input type="text" value={form.mobileImage} onChange={(e) => setForm({ ...form, mobileImage: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" placeholder="/banners/mobile.jpg" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">Link ka</label>
                  <select value={form.linkType} onChange={(e) => setForm({ ...form, linkType: e.target.value as Banner["linkType"] })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none">
                ...
                <div>
                  ...
                  <input type="text" value={form.linkUrl} onChange={(e) => setForm({ ...form, linkUrl: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">CTA tekst</label>
                  <input type="text" value={form.ctaText} onChange={(e) => setForm({ ...form, ctaText: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" placeholder="Pogledaj ponudu" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">Ciljna grupa</label>
                  <select value={form.target} onChange={(e) => setForm({ ...form, target: e.target.value as Banner["target"] })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="date" value={form.activeFrom} onChange={(e) => setForm({ ...form, activeFrom: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">Aktivno do</label>
                  <input type="date" value={form.activeTo} onChange={(e) => setForm({ ...form, activeTo: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">Redosled prikaza</label>
                  <input type="number" value={form.displayOrder} onChange={(e) => setForm({ ...form, displayOrder: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" />
                </div>
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 p-6 border-t border-[#e5e5e5]">
              ...
              <button onClick={handleSave} className="px-5 py-2.5 bg-[#c8a96e] text-white rounded-lg text-sm font-medium hover:bg-[#b8994e] transition-colors">
