...
const TiptapEditor = dynamic(() => import("@/components/admin/TiptapEditor"), {
  ...
  loading: () => (
    <div className="border border-stone-200 rounded-lg p-16 flex items-center justify-center bg-white">
      <Loader2 size={24} className="animate-spin text-[#2e2e2e]" />
    </div>
  ),
});
    ...
    return (
      <div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <input
                ...
                className="font-serif text-xl lg:text-2xl font-bold text-black bg-transparent border-0 border-b border-transparent hover:border-stone-300 focus:border-[#e1dbd0] focus:outline-none w-full pb-0.5 transition-colors"
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-5 py-2.5 text-sm font-medium text-[#2e2e2e] hover:text-black hover:bg-stone-100 rounded-lg transition-colors"
            >
              Otkaži
            </button>
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#e1dbd0] hover:bg-[#6a624f] text-[#2e2e2e] rounded-lg text-sm font-semibold transition-colors disabled:opacity-40 shadow-sm"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-6">
          <div>
            <label className="block text-xs font-medium text-[#2e2e2e] uppercase tracking-wider mb-1">Predmet emaila (Subject)</label>
            <input
              type="text"
              value={templateForm.subject}
              onChange={(e) => setTemplateForm({ ...templateForm, subject: e.target.value })}
              className="w-full px-3.5 py-2.5 bg-white border border-stone-200 rounded-lg text-sm focus:border-[#e1dbd0] focus:ring-1 focus:ring-[#e1dbd0]/20 focus:outline-none transition-colors"
              placeholder="Subject line za email..."
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-[#2e2e2e] uppercase tracking-wider mb-1">Opis šablona (opciono)</label>
            <input
              type="text"
        ...
        <div className="mb-6">
          <button
            ...
            className="flex items-center gap-2 text-sm font-medium text-[#2e2e2e] hover:text-black transition-colors"
          >
            <Settings2 size={16} />
          ...
          {showEmailSettings && (
            <div className="mt-3 grid grid-cols-1 lg:grid-cols-2 gap-4 p-5 bg-white border border-stone-200 rounded-lg">
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">Zaglavlje (Header)</h4>
                <div>
                  <label className="block text-xs text-[#2e2e2e] mb-1">Naslov</label>
                  <input
                    type="text"
                    value={emailOptions.headerTitle || ""}
                    onChange={(e) => setEmailOptions({ ...emailOptions, headerTitle: e.target.value })}
                    className="w-full px-3 py-2 bg-white border border-stone-200 rounded-lg text-sm focus:border-[#e1dbd0] focus:ring-1 focus:ring-[#e1dbd0]/20 focus:outline-none"
                    placeholder="ALTAMODA"
                  />
                </div>
                <div>
                  <label className="block text-xs text-[#2e2e2e] mb-1">Podnaslov</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs text-[#2e2e2e] mb-1">Boja pozadine</label>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={emailOptions.headerBg || "#2e2e2e"}
                      onChange={(e) => setEmailOptions({ ...emailOptions, headerBg: e.target.value })}
                      className="flex-1 px-3 py-2 bg-white border border-stone-200 rounded-lg text-sm font-mono focus:border-[#e1dbd0] focus:ring-1 focus:ring-[#e1dbd0]/20 focus:outline-none"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-xs text-[#2e2e2e] mb-1">Logo slika (opciono — zamenjuje tekst)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs text-[#2e2e2e] mb-1">Pozadinska slika zaglavlja (opciono)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs text-[#2e2e2e] mb-1">Pozadinska slika tela emaila (za Info šablon, opciono)</label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={emailOptions.bodyBgImage || ""}
                      onChange={(e) => setEmailOptions({ ...emailOptions, bodyBgImage: e.target.value })}
                      className="flex-1 px-3 py-2 bg-white border border-stone-200 rounded-lg text-sm focus:border-[#e1dbd0] focus:ring-1 focus:ring-[#e1dbd0]/20 focus:outline-none"
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-[#e1dbd0] text-[#2e2e2e] hover:bg-[#e1dbd0] hover:text-[#2e2e2e] rounded-lg text-xs font-semibold transition-colors disabled:opacity-50 whitespace-nowrap"
                    >
                  ...
                  {emailOptions.bodyBgImage && (
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="text-xs text-[#2e2e2e] hover:text-red-600 underline"
                      >
                        Ukloni
                      </button>
                    </div>
              ...
              {/* Footer settings */}
              <div className="space-y-3">
                <h4 className="text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">Podnožje (Footer)</h4>
                <div>
                  <label className="block text-xs text-[#2e2e2e] mb-1">Tekst podnožja</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs text-[#2e2e2e] mb-1">Copyright</label>
                  <input
                    type="text"
        ...
        <div className="mb-6 p-4 bg-white border border-stone-200 rounded-lg">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
            ...
            <div className="lg:col-span-3">
              <label className="block text-xs font-medium text-[#2e2e2e] uppercase tracking-wider mb-1">Publika</label>
              <select
                value={sendSegment}
            ...
            <div className="lg:col-span-2">
              <div className="flex items-center gap-2 px-3.5 py-2.5 bg-stone-50 border border-stone-200 rounded-lg">
                <Users size={16} className="text-[#2e2e2e]" />
                <span className="text-sm font-semibold text-[#2e2e2e]">{sendRecipientCount}</span>
                <span className="text-xs text-[#2e2e2e]">prim.</span>
              </div>
            </div>
            <div className="lg:col-span-3">
              <button
          ...
          <div className="mt-3 pt-3 border-t border-stone-200">
            <label className="block text-xs font-medium text-[#2e2e2e] uppercase tracking-wider mb-1">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                className="flex-1 px-3.5 py-2.5 bg-white border border-stone-200 rounded-lg text-sm focus:border-[#e1dbd0] focus:ring-1 focus:ring-[#e1dbd0]/20 focus:outline-none"
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-[#e1dbd0] text-[#2e2e2e] hover:bg-[#e1dbd0] hover:text-[#2e2e2e] rounded-lg text-sm font-semibold transition-colors disabled:opacity-40"
              >
                {inlineTestSending ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}
                {inlineTestSending ? "Slanje..." : "Pošalji test"}
              </button>
            </div>
            <p className="mt-1 text-[11px] text-[#2e2e2e]">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">Editor</h3>
              <span className="text-[10px] text-[#2e2e2e]">Pišite tekst, dodajte slike i formatirajte sadržaj</span>
            </div>
            <TiptapEditor
              content={editorBodyContent}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs font-semibold text-[#2e2e2e] uppercase tracking-wider">Pregled emaila</h3>
              <span className="text-[10px] font-semibold text-[#2e2e2e] bg-[#e1dbd0]/10 px-2 py-0.5 rounded-full uppercase tracking-wider">
                Uživo
              </span>
            </div>
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-[#2e2e2e] mt-1">{t("newsletter.subtitle")}</p>
        </div>
        <button
          onClick={() => { setShowTestModal(true); setTestResult(null); setTestEmail(""); }}
          className="flex items-center gap-2 px-4 py-2.5 bg-[#e1dbd0] hover:bg-[#6a624f] text-[#2e2e2e] text-sm font-medium rounded-sm transition-colors"
        >
      ...
      <div className="flex gap-1 bg-stone-100 rounded-sm p-1 mb-6 w-fit">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === tab.id ? "bg-white text-black shadow-sm" : "text-[#2e2e2e] hover:text-[#2e2e2e]"}`}
          >
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === "templates" && (
        <div>
          <div className="flex items-center justify-between mb-6">
            <p className="text-sm text-[#2e2e2e]">Upravljajte email šablonima za vaše newsletter kampanje.</p>
          </div>
          ...
          {templatesLoading || seeding ? (
            <div className="bg-white rounded-sm border border-stone-200 p-12 flex flex-col items-center justify-center">
              <Loader2 size={28} className="animate-spin text-[#2e2e2e] mb-3" />
              <span className="text-[#2e2e2e] text-sm">{seeding ? "Kreiranje podrazumevanih šablona..." : "Učitavanje šablona..."}</span>
            </div>
          ) : templates.length === 0 ? (
            <div className="bg-white rounded-sm border border-stone-200 p-12 text-center">
              <FileText size={40} className="mx-auto text-[#2e2e2e] mb-3" />
              <p className="text-[#2e2e2e]">Nema šablona. Kliknite &quot;Novi šablon&quot; da kreirate prvi.</p>
            </div>
          ) : (
              ...
              {templates.map((template) => (
                ...
                >
                  ...
                  >
                    ...
                    {template.isDefault && (
                      <span className="absolute top-2.5 right-2.5 px-2.5 py-0.5 bg-[#e1dbd0]/90 text-[#2e2e2e] text-[10px] font-semibold rounded-full uppercase tracking-wider backdrop-blur-sm">
                        Podrazumevani
                      </span>
                    )}
                  ...
                  <div className="p-4">
                    ...
                    {template.description && (
                      <p className="text-xs text-[#2e2e2e] mb-2.5 line-clamp-2">{template.description}</p>
                    )}
                    <p className="text-xs text-[#2e2e2e] mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => openTemplateEditor(template)}
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-[#e1dbd0] text-[#2e2e2e] rounded-lg text-xs font-semibold hover:bg-[#6a624f] transition-colors"
                      >
      ...
      {activeTab === "campaigns" && (
        <div>
          <div className="flex items-center justify-between mb-6">
            <p className="text-sm text-[#2e2e2e]">Pregled svih poslatih i tekućih newsletter kampanja.</p>
            <button
              onClick={fetchCampaigns}
              className="text-xs text-[#2e2e2e] hover:text-black underline"
            >
              Osveži
            </button>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-stone-100 border-b border-stone-200">
                  <tr>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">Naslov</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">Predmet</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">Segment</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">Status</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">Poslato</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">Datum</th>
                  </tr>
                </thead>
                <tbody>
                  {campaignsLoading ? (
                    <tr><td colSpan={6} className="px-4 py-8 text-center text-[#2e2e2e]">Učitavanje...</td></tr>
                  ) : campaigns.length === 0 ? (
                    <tr><td colSpan={6} className="px-4 py-8 text-center text-[#2e2e2e]">Još nema poslatih kampanja.</td></tr>
                  ) : (
                    campaigns.map((c) => {
                      ...
                      return (
                        <tr key={c.id} className="border-b border-stone-100 hover:bg-stone-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-[#2e2e2e]">{c.subject}</td>
                          <td className="px-4 py-3">
                            <span className="text-xs uppercase tracking-wider text-[#2e2e2e]">{c.segment}</span>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 text-[#2e2e2e]">{c.sentCount}</td>
                          <td className="px-4 py-3 text-[#2e2e2e] text-xs">
                            {c.sentAt
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-sm border border-stone-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-[#2e2e2e] uppercase tracking-wider">{t("newsletter.totalActive")}</p>
            ...
            <div className="bg-white rounded-sm border border-stone-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-[#2e2e2e] uppercase tracking-wider">{t("newsletter.b2bSubscribers")}</p>
            ...
            <div className="bg-white rounded-sm border border-stone-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-[#2e2e2e] uppercase tracking-wider">{t("newsletter.b2cSubscribers")}</p>
          ...
          <div className="bg-white rounded-sm border border-stone-200 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-stone-100 border-b border-stone-200">
                  <tr>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">{t("newsletter.email")}</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">{t("newsletter.segment")}</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">{t("newsletter.subscribeDate")}</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">{t("newsletter.status")}</th>
                    <th className="text-left px-4 py-3 font-semibold text-[#2e2e2e]">{t("newsletter.actions")}</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-[#2e2e2e]">{t("newsletter.loading")}</td></tr>
                  ) : subscribers.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-[#2e2e2e]">{t("newsletter.noSubscribers")}</td></tr>
                  ) : (
                    subscribers.map((sub) => (
                      <tr key={sub.id} className="border-b border-stone-100 hover:bg-stone-100 transition-colors">
                        ...
                        <td className="px-4 py-3 text-[#2e2e2e]">{new Date(sub.subscribedAt).toLocaleDateString("sr-RS")}</td>
                        <td className="px-4 py-3">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-stone-200">
                <span className="text-sm text-[#2e2e2e]">{t("newsletter.pageOf")} {page} {t("newsletter.of")} {totalPages} ({total} {t("newsletter.totalLabel")})</span>
                <div className="flex items-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowNewTemplateModal(false)}>
          <div className="bg-white rounded-sm w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-end gap-3 p-6 border-t border-stone-200">
              ...
              <button onClick={handleCreateTemplate} disabled={!templateForm.name.trim() || !templateForm.subject.trim() || templateSaving} className="inline-flex items-center gap-2 px-5 py-2.5 bg-black text-white rounded-sm text-sm font-medium hover:bg-[#e1dbd0] hover:text-[#2e2e2e] transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowTestModal(false)}>
          <div className="bg-white rounded-sm max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowTestModal(false)} className="text-[#2e2e2e] hover:text-black"><X className="w-5 h-5" /></button>
            </div>
            <p className="text-sm text-[#2e2e2e] mb-4">Unesite email adresu na koju želite da pošaljete probni newsletter email.</p>
            <input
              type="email"
              value={testEmail}
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowTestModal(false)} className="flex-1 px-4 py-2.5 border border-stone-200 text-[#2e2e2e] rounded-sm text-sm font-medium hover:bg-stone-50 transition-colors">Otkaži</button>
              <button onClick={handleSendTest} disabled={testSending || !testEmail.trim()} className="flex-1 px-4 py-2.5 bg-[#e1dbd0] hover:bg-[#6a624f] text-[#2e2e2e] rounded-sm text-sm font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
