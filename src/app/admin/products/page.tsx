  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-serif font-bold text-[#1a1a1a]">Proizvodi</h1>
      ...
      <div className="bg-white rounded-xl border border-[#e5e5e5] p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className="sm:hidden flex items-center gap-2 px-4 py-2.5 bg-[#f5f5f5] rounded-lg text-sm text-[#666]"
          >
            <Filter size={16} />
            Filteri
          </button>
          <div className={`${showFilters ? "flex" : "hidden"} sm:flex flex-col sm:flex-row gap-3`}>
            <select value={brandFilter} onChange={(e) => { setBrandFilter(e.target.value); setCurrentPage(1); }} className="px-3 py-2.5 bg-[#f5f5f5] border border-transparent rounded-lg text-sm cursor-pointer focus:border-[#c8a96e]">
              {brands.map((b) => <option key={b}>{b}</option>)}
            </select>
            <select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); setCurrentPage(1); }} className="px-3 py-2.5 bg-[#f5f5f5] border border-transparent rounded-lg text-sm cursor-pointer focus:border-[#c8a96e]">
              {categories.map((c) => <option key={c}>{c}</option>)}
            </select>
            <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setCurrentPage(1); }} className="px-3 py-2.5 bg-[#f5f5f5] border border-transparent rounded-lg text-sm cursor-pointer focus:border-[#c8a96e]">
          ...
          <div className="mt-3 pt-3 border-t border-[#f0f0f0] flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="px-3 py-1.5 bg-[#1a1a1a] text-white rounded-lg text-sm flex items-center gap-1"
              >
                Akcije <MoreVertical size={14} />
              </button>
              {showBulkMenu && (
                <div className="absolute top-full left-0 mt-1 w-40 bg-white rounded-lg shadow-xl border border-[#e5e5e5] overflow-hidden z-10 animate-slideDown">
                  <button onClick={() => bulkAction("activate")} className="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-[#333] hover:bg-[#f5f5f5]">
                    <Eye size={14} /> Aktiviraj
                  </button>
                  <button onClick={() => bulkAction("deactivate")} className="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-[#333] hover:bg-[#f5f5f5]">
            ...
            <thead>
              <tr className="bg-[#fafafa] border-b border-[#e5e5e5]">
                <th className="px-4 py-3 text-left">
                  <button onClick={selectAll} className="text-[#999] hover:text-[#1a1a1a]">
              ...
              {paginated.map((product) => (
                <tr key={product.id} className="hover:bg-[#fafafa] transition-colors">
                  <td className="px-4 py-3">
                    <button onClick={() => toggleSelect(product.id)} className="text-[#999] hover:text-[#1a1a1a]">
                      {selectedIds.includes(product.id) ? <CheckSquare size={18} className="text-[#c8a96e]" /> : <Square size={18} />}
                    </button>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div>
                      <span className="text-sm font-medium text-[#1a1a1a]">{product.price.toLocaleString()} RSD</span>
                      {product.oldPrice && (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEditModal(product)} className="p-1.5 text-[#999] hover:text-[#c8a96e] hover:bg-[#c8a96e]/10 rounded-lg transition-colors">
                        <Edit3 size={15} />
                      </button>
          ...
          <div className="px-6 py-4 border-t border-[#e5e5e5] flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-2 rounded-lg text-[#666] hover:bg-[#f5f5f5] disabled:opacity-30 disabled:cursor-not-allowed"
              >
                <ChevronLeft size={18} />
              </button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${page === currentPage ? "bg-[#c8a96e] text-white" : "text-[#666] hover:bg-[#f5f5f5]"}`}
                >
                  {page}
                </button>
              ))}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scaleIn" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white p-6 border-b border-[#e5e5e5] flex items-center justify-between rounded-t-2xl">
              <h2 className="text-lg font-serif font-bold text-[#1a1a1a]">
                {editingProduct ? "Izmeni Proizvod" : "Dodaj Proizvod"}
              </h2>
              <button onClick={() => setShowModal(false)} className="p-1 text-[#999] hover:text-[#1a1a1a]">
                <X size={20} />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm" placeholder="Unesite naziv..." />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1.5">Brend</label>
                  <select value={formData.brand} onChange={(e) => setFormData({ ...formData, brand: e.target.value })} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm cursor-pointer">
                ...
                <div>
                  ...
                  <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm cursor-pointer">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm" placeholder="0" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1.5">Stara cena (RSD)</label>
                  <input type="number" value={formData.oldPrice} onChange={(e) => setFormData({ ...formData, oldPrice: e.target.value })} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm" placeholder="Opciono" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-[#333] mb-1.5">Opis</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm resize-none" placeholder="Opis proizvoda..." />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1.5">Zalihe</label>
                  <input type="number" value={formData.stock} onChange={(e) => setFormData({ ...formData, stock: e.target.value })} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm" placeholder="0" />
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <button
                      ...
                      className={`relative w-12 h-6 rounded-full transition-colors ${formData.status === "active" ? "bg-[#c8a96e]" : "bg-gray-300"}`}
                    >
              ...
              <div>
                ...
                <input type="text" value={formData.image} onChange={(e) => setFormData({ ...formData, image: e.target.value })} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm" placeholder="/products/slika.jpg" />
              </div>
            </div>
            <div className="p-6 border-t border-[#e5e5e5] flex items-center justify-end gap-3">
              <button onClick={() => setShowModal(false)} className="px-5 py-2.5 rounded-lg text-sm font-medium text-[#666] hover:bg-[#f5f5f5] transition-colors">
                Otkaži
              </button>
