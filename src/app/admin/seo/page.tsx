  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-serif text-2xl lg:text-3xl font-bold text-[#1a1a1a]">SEO</h1>
            ...
            <div className="bg-white rounded-xl border border-[#e5e5e5] overflow-hidden">
              <div className="p-3 border-b border-[#e5e5e5]">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Pretraži..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none" />
                </div>
              </div>
              <div className="divide-y divide-[#f5f5f5]">
                {items.map((item) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-[#fafafa] transition-colors flex items-center justify-between ${editingItem?.id === item.id ? "bg-[#c8a96e]/5 border-l-2 border-[#c8a96e]" : ""}`}
                  >
            ...
            {editingItem ? (
              <div className="bg-white rounded-xl border border-[#e5e5e5] overflow-hidden">
                <div className="p-4 border-b border-[#e5e5e5] flex items-center justify-between">
                  <h3 className="font-semibold text-[#1a1a1a]">{editingItem.name}</h3>
                  <button onClick={closeEdit} className="p-1 hover:bg-[#f5f5f5] rounded-lg">
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none resize-none"
                    />
                  </div>
                  ...
                  <div className="flex justify-end pt-2">
                    <button className="inline-flex items-center gap-2 bg-[#c8a96e] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-[#b8994e] transition-colors">
                      <Save size={16} />
                      Sačuvaj SEO
                    </button>
                  </div>
              ...
              <div className="bg-white rounded-xl border border-[#e5e5e5] p-10 text-center">
                <Globe size={48} className="mx-auto text-[#c8a96e]/30 mb-3" />
      ...
      {activeTab === "global" && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
            <h3 className="font-semibold text-[#1a1a1a] mb-4">Šabloni</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#333] mb-1">Podrazumevani šablon naslova</label>
                <input type="text" value={globalSettings.titleTemplate} onChange={(e) => setGlobalSettings({ ...globalSettings, titleTemplate: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none font-mono" />
              ...
              <div>
                ...
                <textarea value={globalSettings.metaTemplate} onChange={(e) => setGlobalSettings({ ...globalSettings, metaTemplate: e.target.value })} rows={2} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none resize-none font-mono" />
              </div>
            </div>
          </div>
          ...
          {/* Robots.txt */}
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
            <h3 className="font-semibold text-[#1a1a1a] mb-4">Robots.txt</h3>
            <textarea
              value={globalSettings.robotsTxt}
              onChange={(e) => setGlobalSettings({ ...globalSettings, robotsTxt: e.target.value })}
              rows={6}
              className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none resize-none font-mono"
            />
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-[#1a1a1a]">Sitemap</h3>
                <p className="text-sm text-[#666] mt-1">Poslednje generisano: 2026-03-15 08:00</p>
              </div>
              <button className="inline-flex items-center gap-2 px-4 py-2 bg-[#f5f5f5] border border-[#e5e5e5] rounded-lg text-sm font-medium hover:bg-[#eee] transition-colors">
                <RefreshCw size={16} />
                Generiši sitemap
              </button>
            </div>
          ...
          {/* Tracking IDs */}
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-5">
            <h3 className="font-semibold text-[#1a1a1a] mb-4">Praćenje i analitika</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#333] mb-1">Google Analytics ID</label>
                <input type="text" value={globalSettings.gaId} onChange={(e) => setGlobalSettings({ ...globalSettings, gaId: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none font-mono" placeholder="G-XXXXXXXXXX" />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#333] mb-1">Facebook Pixel ID</label>
                <input type="text" value={globalSettings.fbPixelId} onChange={(e) => setGlobalSettings({ ...globalSettings, fbPixelId: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none font-mono" placeholder="1234567890" />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#333] mb-1">Google Tag Manager ID</label>
                <input type="text" value={globalSettings.gtmId} onChange={(e) => setGlobalSettings({ ...globalSettings, gtmId: e.target.value })} className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#c8a96e] focus:outline-none font-mono" placeholder="GTM-XXXXXXX" />
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <button className="inline-flex items-center gap-2 bg-[#c8a96e] text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-[#b8994e] transition-colors">
              <Save size={16} />
