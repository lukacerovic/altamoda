...
export default function BundlesPage() {
  ...
  const statusBadge = (status: string) => {
    ...
    const labels: Record<string, string> = { active: t("admin.active"), expired: t("admin.expired"), scheduled: t("admin.scheduled") };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-serif text-2xl lg:text-3xl font-bold text-[#2d2d2d]">{t("admin.bundles")}</h1>
          ...
          <div key={bundle.id} className="bg-white rounded-xl border border-[#e0d8cc] overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div className="p-5">
              ...
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs bg-[#f5f0e8] text-[#666] px-2 py-1 rounded">{bundle.products.length} {t("admin.productsCount")}</span>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-[#e0d8cc]">
              <h2 className="font-serif text-xl font-bold text-[#2d2d2d]">{t("admin.newBundle")}</h2>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="relative mb-2">
                  ...
                  <input type="text" value={productSearch} onChange={(e) => setProductSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 border border-[#e0d8cc] rounded-lg text-sm focus:border-[#8c4a5a] focus:outline-none" placeholder={t("admin.searchProductsList")} />
                </div>
            ...
            <div className="flex items-center justify-end gap-3 p-6 border-t border-[#e0d8cc]">
              <button onClick={() => setShowModal(false)} className="px-5 py-2.5 border border-[#e0d8cc] rounded-lg text-sm font-medium hover:bg-[#f5f0e8] transition-colors">{t("admin.cancel")}</button>
              <button onClick={handleCreate} className="px-5 py-2.5 bg-[#8c4a5a] text-white rounded-lg text-sm font-medium hover:bg-[#b8994e] transition-colors">{t("admin.createBundle")}</button>
            </div>
          </div>
        </div>
      )}
