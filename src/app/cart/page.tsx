      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {items.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-[#2e2e2e] mb-6">{t("cart.emptyDesc")}</p>
                ...
                <div key={item.productId} className={`bg-white rounded-sm shadow-sm p-4 md:p-6 flex gap-4 ${outOfStock ? "opacity-60" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button onClick={() => removeItem(item.productId)} className="text-[#2e2e2e] hover:text-[#b5453a] transition-colors flex-shrink-0"><Trash2 className="w-4 h-4" /></button>
                    </div>
                    <div className="flex items-center justify-between mt-4">
                      {outOfStock ? (
                        <span className="text-xs text-[#2e2e2e] italic">{t("cart.unavailable")}</span>
                      ) : (
                      ...
                      <span className={`font-bold ${outOfStock ? "text-[#2e2e2e] line-through" : "text-[#2e2e2e]"}`}>{(item.price * item.quantity).toLocaleString("sr-RS")} RSD</span>
                    </div>
                  </div>
                </div>
                );
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      <div className="flex items-center gap-2">
                        <FileText className="w-4 h-4 text-[#2e2e2e]" />
                        <span className="text-sm text-[#2e2e2e]">{t("cart.invoicePayment")}</span>
                      </div>
                    </label>
                    <div className="bg-[#FFFFFF] rounded p-3 text-xs text-[#2e2e2e] flex items-start gap-2">
            ...
            <div className="flex flex-col">
              <div className="bg-white rounded-sm shadow-sm p-6 flex-1 flex flex-col">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between"><span className="text-[#2e2e2e]">{t("cart.subtotal")}</span><span className="font-medium">{subtotal.toLocaleString("sr-RS")} RSD</span></div>
                  <div className="flex justify-between"><span className="text-[#2e2e2e]">{t("cart.delivery")}</span><span className="text-xs text-[#2e2e2e]">{t("cart.selectAtCheckout") ?? "U sledećem koraku"}</span></div>
                </div>
                ...
                <div className="mt-4 space-y-2 text-xs text-[#2e2e2e]">
                ...
                <div className="flex items-center gap-3 mt-4 pt-4 border-t border-[#D8CFBC]">
                  {["Visa", "Mastercard", "PayPal", "Pouzece"].map((p) => (
                    <span key={p} className="px-2 py-1 bg-[#FFFFFF] rounded text-[10px] text-[#2e2e2e] font-medium">{p}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            {recommended.map((p) => (
              <Link key={p.id} href={`/products/${p.id}`} className="product-card bg-white rounded-[4px] shadow-sm hover:shadow-md transition-all group overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-0.5 mt-2">{[...Array(5)].map((_, i) => <Star key={i} className={`w-3 h-3 ${i < p.rating ? "fill-[#e1dbd0] text-secondary" : "text-[#D8CFBC]"}`} />)}</div>
