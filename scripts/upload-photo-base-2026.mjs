...
fs.writeFileSync(
  "scripts/photo-upload-plan.json",
  JSON.stringify(
    {
      folder: FOLDER,
      total: files.length,
...
console.log("Plan -> scripts/photo-upload-plan.json");
...
fs.writeFileSync("scripts/cloudinary-url-map-2026.json", JSON.stringify(urlByPublicId, null, 2));
fs.writeFileSync("scripts/cloudinary-urls-by-code-2026.json", JSON.stringify(urlByCode, null, 2));
console.log(`\nDone! uploaded/existed=${ok} failed=${fail}`);
console.log("Maps -> scripts/cloudinary-url-map-2026.json, scripts/cloudinary-urls-by-code-2026.json");
